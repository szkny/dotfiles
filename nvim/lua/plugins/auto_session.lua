return {
  ...
  config = function(_, opts)
    ...
    local function find_buffer_by_filetype(ftype)
      for _, win in ipairs(vim.api.nvim_list_wins()) do
        local buf = vim.api.nvim_win_get_buf(win)
        if vim.bo[buf].filetype == ftype then
          return true
        end
      end
      return false
    ...
    local function close_sidekick()
      if find_buffer_by_filetype("sidekick_terminal") then
        require("sidekick.cli").close()
        auto_session_plugins.sidekick.state = true
      end
    end
